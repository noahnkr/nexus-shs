"""Edge security primitives (spec §5.3) — generic, shared across all connectors.
...
def within_window(signed_ts: float, window_seconds: int = 300, *, now: float | None = None) -> bool:
    """True if `signed_ts` is within ±window of now — rejects replays (§5.3)."""
    now = now if now is not None else time.time()
    return abs(now - signed_ts) <= window_seconds


class SeenCache:
    """In-memory TTL set keyed by `source:external_id` for idempotency (§5.3).
