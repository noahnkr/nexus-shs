...
. The tier is passed to the agent as authoritative context; the model never decides
...
you've proven safe (the trust ratchet).

Tier semantics map to the one trust rule:
...
# One row per (source, kind) that matters; unknown pairs fail safe to supervised.
