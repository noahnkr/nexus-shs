...
| `nexus/connectors/goto_connect/` | GoTo Connect phone system: WebSocket push consumer (`stream.py`, lifespan task), frame→Stimulus mapping (`events.py`), read-only OAuth client + loop/MCP tools (`client.py`), missed-call gap-fill (`sync.py`) — see `docs/connectors/goto-connect.md` |
